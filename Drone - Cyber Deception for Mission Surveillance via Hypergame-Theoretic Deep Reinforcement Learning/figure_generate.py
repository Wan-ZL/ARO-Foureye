...
def all_none_trial_finder_2(time_vary_set, budget_vary_set, HD_vary_set, scheme, data_folder_name='data', defense_strategy=0, HEU_guilded=False):
    '''
    ...
    for time_val in time_vary_set:
        ...
        for budget_val in budget_vary_set:
            ...
            for HD_val in HD_vary_set:
                ...
                print("searching", setting_folder)
                path_list = glob.glob(data_folder_name + '/' + setting_folder + '/scheme_' + scheme + '/HEU_guilded=' + str(HEU_guilded) + '/*-eps')
                value_set = []
                path_set = []
                # max_value = 0
                # max_path = None
...
def draw_all_scheme_sens_analysis_2(tag_name, vary_time=True, defense_strategy=0, HEU_guilded=False):
    '''
    ...
    if vary_time:
        ...
        time_vary_set=[150]#[30]
        budget_vary_set = [5]#[1, 2, 3, 4, 5]
    ...
    for scheme_name in scheme_name_set:
        all_scheme_path_set[schemeShort2schemeLong[scheme_name]] = all_none_trial_finder_2(time_vary_set,
                                                                                           ...
                                                                                           scheme_name, data_folder_name, defense_strategy, HEU_guilded)
...
def draw_def_strategies_compare_analysis(tag_name, vary_time=True):
    ...
    for key in all_scheme_path_set.keys():
        for HD_val in HD_vary_dict[key]:
            ...
            print("x_set", x_set)
            print("y_set", y_set)
            line, = plt.plot(x_set, y_set, linestyle=linestyle_list[x_counter], linewidth=figure_linewidth, markersize=marker_size,
                     marker=marker_list[x_counter], label=scheme2stringShort[key]+HD_str)
            lines.append(line)
            x_counter += 1
...
def draw_accum_reward(tag_name, scheme_name_set=["att", "DefAtt"]):
    ...
    for scheme_name in scheme_name_set:
        all_scheme_path_set[schemeShort2schemeLong[scheme_name]] = all_none_trial_finder_2(time_vary_set,
                                                                                           budget_vary_set, HD_vary_set,
                                                                                           scheme_name, data_folder_name='data')
    # Draw figure
    epi_size = 5000
    lines = []
    print("Drawing:", tag_name)
...
if __name__ == '__main__':
    ...
    # Figure for testing
    tag_set = ["Accumulated Reward Defender", "Defender's Total Loss"]
    for tag in tag_set:
        p1 = Process(target=draw_all_scheme_sens_analysis_2, args=(tag, False, 0, False))
        p1.start()
        # p2 = Process(target=draw_all_scheme_sens_analysis_2, args=(tag, False, 0, True))
        # p2.start()
        # p3 = Process(target=draw_all_scheme_sens_analysis_2, args=(tag, False, 0, False))
        # p3.start()
        # p4 = Process(target=draw_all_scheme_sens_analysis_2, args=(tag, False, 0, True))
        # p4.start()
