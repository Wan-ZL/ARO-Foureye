...
class defender_model(player_model):
    def __init__(self, system):
        ...
        self.strategy = 9   # index range (0,9) maps to defense strategy 1 to 10 in the paper
    ...
    def update_MD_next_destination(self):
        for MD in self.system.MD_set:
            ...
            map_x_index = round(MD.xyz_destination[0] - 1)        # minus 1 for ignoring GCS
            map_y_index = round(MD.xyz_destination[1] - 1)        # minus 1 for ignoring GCS
    ...
    def update_HD_next_destination(self):
        ...
        for HD in self.system.HD_set:
            ...
            for MD_id in L_MD_set:
                ...
                if self.system.drones_distance(self.system.MD_dict[MD_id].xyz_destination, self.system.HD_dict[HD.ID].xyz_destination) < p_H_r:
            ...
            if N_l_H_set.size < self.tau_lower:
                ...
                for MD_id_candi in L_MD_set:  # search MD position that HD can move to so more MD can be protected
                    temp_set = np.empty(0)
                    for MD_id in L_MD_set:
                        temp_position = self.system.MD_dict[MD_id].xyz_destination
                        temp_position[:2] = self.system.MD_dict[MD_id].xyz_destination[:2]
                        # if drones_distance(Desti_XYZ[MD_id_candi], Desti_XYZ[MD_id]) < p_H_r:
                        if self.system.drones_distance(self.system.MD_dict[MD_id_candi].xyz_destination, self.system.MD_dict[MD_id].xyz_destination) < p_H_r:
                            temp_set = np.append(temp_set, MD_id)
                    if temp_set.size > N_l_H_new_set.size:  # set new position as candidate
                        N_l_H_new_set = temp_set
                        HD_pos_candidate = self.system.MD_dict[MD_id_candi].xyz_destination
    ...
    def MD_position_enumerate(self):
        ...
        for MD in self.system.MD_set:
            if MD.crashed:
                continue
            xy_list = MD.xyz_destination[:2].astype(int).tolist()
