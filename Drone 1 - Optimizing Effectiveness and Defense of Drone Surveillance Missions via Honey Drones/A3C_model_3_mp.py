...
class ActorCritic(nn.Module):
    def __init__(self, input_dims, n_actions, epsilon=0.1, epsilon_decay=0.9, pi_net_struc=None, v_net_struct=None, fixed_seed=True):
        ...
        # print("self.pi_net", [module for module in self.pi_net.modules() if not isinstance(module, nn.Sequential)])
        # self.pi_layers = self.build_Layers(input_dims, n_actions, pi_net_struc)
        # self.v_layers = self.build_Layers(input_dims, 1, v_net_struct)

        self.pi_net.apply(self.init_weight_bias_per_layer)  # initial weight (normal distribution)
        self.v_net.apply(self.init_weight_bias_per_layer)  # initial weight (normal distribution)
        # self.init_weight_bias(self.pi_layers)
        # self.init_weight_bias(self.v_layers)
    ...
    def forward(self, state):
        pi_layers = [module for module in self.pi_net.modules() if not isinstance(module, nn.Sequential)]
        pi_x = torch.clone(state)
        for layer in pi_layers:
            pi_x = layer(pi_x)

        v_layers = [module for module in self.v_net.modules() if not isinstance(module, nn.Sequential)]
        v_x = torch.clone(state)
    ...
    def init_weight_bias(self, layers):
        for layer in layers:
            self.init_weight_bias_per_layer(layer)

    def init_weight_bias_per_layer(self, layer):
    ...
    def build_Layers(self, obser_space, action_space, net_struc):
        layers = []
        in_features = obser_space
        # for i in range(n_layers):
        for node_num in net_struc:
...
def run_Agent(gnet, opt, shared_dict, lr_decay, gamma, epsilon, epsilon_decay, MAX_EP, fixed_seed, trial, name_id: int, player='def', exist_model=False, is_custom_env=True):
    agent = Agent(gnet, opt, shared_dict, lr_decay, gamma, epsilon, epsilon_decay, MAX_EP, fixed_seed, trial, name_id, player, exist_model, is_custom_env)
    agent.run()