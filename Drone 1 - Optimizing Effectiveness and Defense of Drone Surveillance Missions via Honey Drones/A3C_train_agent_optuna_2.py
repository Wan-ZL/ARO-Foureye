'''
Project     ：gym-drones 
File        ：A3C_train_agent_optuna_2.py
Author      ：Zelin Wan
Date        ：9/3/22
Description : 
'''
...
def objective(trial, fixed_seed=True, on_server=True):
    ...
    config = dict(glob_episode_thred=10000, min_episode=1000, gamma=0.99, lr=0.0001, LR_decay=1, epsilon = 0,
    ...
    # scheduler = LambdaLR(optim, lr_lambda=lambda epoch: config['LR_decay'] ** epoch)
    ...
    workers = [Agent(global_actor_critic=global_actor_critic,
                     optim=optim,
                     # scheduler=scheduler,
