'''
Project     ：gym-drones 
File        ：A3C_train_agent_optuna_3_mp.py
Author      ：Zelin Wan
Date        ：10/10/22
Description : 
'''
...
def objective(trial, fixed_seed=True, on_server=True, exist_model=False, is_defender=True, is_custom_env=True):
    ...
    processes = []
    for name_id in range(num_worker):
        p = mp.Process(target=run_Agent, args=(glob_AC, optim, shared_dict, config["LR_decay"], config["gamma"], config["epsilon"], config["epsilon_decay"], config["glob_episode_thred"], fixed_seed, trial, name_id, player_name, exist_model, is_custom_env))
        p.start()
        processes.append(p)
    for p in processes:
        p.join()
